...
document.addEventListener("DOMContentLoaded", function() {
    ...
    for (var i = 0; i < localStorage.length; i++){
        appendOldNotification(localStorage.getItem(localStorage.key(i)))
    }
...
function appendOldNotification(noti_date) {
    let paragraph = document.querySelector('p#text_option');
    paragraph.style.display = "none";
    ...
    let container = document.querySelector('.noti'); // Seleciona o container correto
        ...
        date.textContent = noti_date; // Adicione a data da notificação
